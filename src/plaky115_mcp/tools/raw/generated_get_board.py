...
def build_tool(client: AsyncPlakyClient) -> ToolSpec:
    async def get_board(
        ...
        try:
            ...
            wire = compact_entity(
                result.model_dump(mode="json", by_alias=True, exclude_none=True), "board"
            )
    ...
    return ToolSpec(
        ...
        title="Get board",
        description="Retrieve a board; it returns the requested result. Requires space ID, board ID and read scope. This operation is read-only.",
        handler=get_board,
