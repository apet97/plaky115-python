...
"""Raw MCP tool for deleteItemComment: Delete item comment."""
...
def build_tool(client: AsyncPlakyClient) -> ToolSpec:
    async def delete_item_comment(
        ...
        try:
            result = await client.comments.delete(
                space_id=spaceId,
                board_id=boardId,
                item_id=itemId,
    ...
    return ToolSpec(
        ...
        description="Delete item comment; it performs the requested change. Requires space ID, board ID, item ID, item comment ID and write and destructive scope. This performs a live write with no dry-run; if a failure is ambiguous, inspect the receipt and do not repeat blindly.",
        handler=delete_item_comment,
        ...
        parameters={
            ...
            "required": ["spaceId", "boardId", "itemId", "itemCommentId"],
        },
    )
